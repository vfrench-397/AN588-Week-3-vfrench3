---
title: "vfrench3_FinalHomeworkCode_02"
...
```{r, cumulative probability function}
...
ppois(q=8, lambda=15, lower.tail = FALSE) #can also argue for the upper tail probability
```

What is the probability that she will hear no calls in a session?

- asking for probability of a singular realization so must call on the d-function which will output a probability density value. 

```{r, probability mass function 1}
dpois(x=0, lambda=15) #if we rounded it to significant decimal places it would be 0% chance. 
```

What is the probability that she will hear exactly 3 calls in a session?

```{r, probablity mass function 2}
dpois(3,15) 
```
...
- The histogram looks very similar to the previously plotted probability mass function indicating the generated data is indeed following a poisson distribution. Some differences are there is not a perfect peak at the lambda p parameter (15) and the plot is no longer perfectly symmetrical, but that is to be expected when working with actual data points over probabilities as probabilities are just an estimate of chance and actual data has a chance for random error. 
